...
var = "pcp.in" #change to name of var in df
...
df.present <- subset(df, CF=="Recent" & season == season)
df.present <- aggregate(PrecipIn~CF ,df.present,mean) #change var
df.future <- subset(df, CF!="Recent" & season == season)
df.future <- aggregate(PrecipIn~CF ,df.future,mean) #change var
